...
public class ViewTools {
  ...
  // 创建弹窗
  public static Dialog createDialogBottomSheet(Context context, boolean canCancel, View view) {
    ScrollView dialogView = ModuleDialogBinding.inflate(LayoutInflater.from(context)).getRoot();
    dialogView.addView(view);

    BottomSheetDialog sheetDialog = new BottomSheetDialog(context);
    sheetDialog.setCancelable(canCancel);
    sheetDialog.setCanceledOnTouchOutside(canCancel);
    sheetDialog.setContentView(dialogView);

    sheetDialog.getWindow().setBackgroundDrawableResource(android.R.color.transparent);
    return sheetDialog;
  }
